...
namespace win.evaluation
{
    public partial class evaluation : Form
    {
        WF.ProjectStatusEnum statu = WF.ProjectStatusEnum.ProjectEvaluation;
        public evaluation()
        {
            InitializeComponent();
        }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            check(WF.ProjectStatusEnum.ProjectStart);
            bind();
        }

        private void check(WF.ProjectStatusEnum statu)
        {
            bool result = true;
            foreach (DataGridViewRow item in dataGridView1.Rows)
            {
                ...
                if ((bool)item.Cells[0].EditedFormattedValue)
                {
                    ...
                    Public.ProjectUtility.updateProjectStatu(projectID, statu);
                    ...
                    {
                        Public.Notice.updateNotice(projectID, statu);
                    }
                    else
                    {
                        result = false;
        ...
        private void btnNG_Click(object sender, EventArgs e)
        {
            ...
            check(WF.ProjectStatusEnum.ProjectNG);
            foreach (DataGridViewRow item in dataGridView1.Rows)
            {
                try
                {
                    ...
                    {
                        //添加审核未通过
                        var model = new Model.notice();
                        model.projectID = item.Cells[1].Value.ToString();
                        model.statu = false;
                        //使用当前流程
                        model.projectStatu = (int)statu;// WF.ProjectStatusEnum.ProjectNG;
                        model.time = DateTime.Now;
                        //获取最后一次状态变更的用户
                        var li = new BLL.MaxStatuChangeBLL().GetModel(p => p.projectID == model.projectID);
                        model.userID = li.userID;
                        new BLL.NoticeBLL().Add(model);
                    }
                }
                catch (Exception ex)
                {
